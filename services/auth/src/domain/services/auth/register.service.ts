...
if(registeredUser.resume_upload_status =='pending' || registeredUser.resume_upload_status =='fail'){
  ...
  async nonBlockingRefreshTokenOps(userId: number , deviceInfo: DeviceInfoType ,userAgent: string ,  rawRefreshToken:string) {
    try {
...
const delay = Math.pow(2, retry) * 1000;
  ...
  async createRefreshToken(userId: number, deviceInfo: DeviceInfoType ,userAgent: string , rawRefreshToken:string){
