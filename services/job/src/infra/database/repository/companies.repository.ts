...
export class PostgresCompaniesRepository implements ICompaniesRepository {
  ...
  async companyDetails(company_id: number, client?: PoolClient): Promise<boolean> {
    ...
    const result = await db.query(
      `SELECT c.*  , COALESCE( jsonb_agg(j.*) FILTER(WHERE j.job_id IS NOT NULL), '[]'::jsonb ) AS jobs FROM companies c LEFT JOIN jobs j ON c.company_id =j.company_id GROUP BY c.company_id HAVING c.company_id = $1`,
      [company_id]
    );
