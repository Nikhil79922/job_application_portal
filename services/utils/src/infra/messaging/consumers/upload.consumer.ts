...
export const startUploadConsumer = async () => {
    ...
    console.log("📥 Upload consumer started");
    ...
    await consumer.run({
        eachMessage: async ({ partition, message }) => {
            ...
            try {
                ...
                // Validate payload
                if (!entityId || !file || !mimeType || !entityType || !uploadType) {
                    console.log("⚠️ Invalid payload", payload);
                    return;
                }
                ...
                const result = await uploadToCloudinary(dataUri, public_id);

                console.log(`☁️ Uploaded → ${result.url}`);

                // Handler map
                const handlers: Record<string, () => Promise<void>> = {
                    "user:resume": async () => {
                        await userRepo.update(entityId, {
                            resume: result.url,
                    ...
                    "company:logo": async () => {
                        await companyRepo.update(entityId, {
                            logo: result.url,
                            logo_public_id: result.public_id,
                            logo_upload_status: "success",
                        });
                    },

                    "user:profile_pic": async () => {
                        await userRepo.update(entityId, {
                            profile_pic: result.url,
                            profile_pic_public_id: result.public_id,
                            profile_pic_upload_status: "success",
                        });
                    },
                };

                const key = `${entityType}:${uploadType}`;

                if (!handlers[key]) {
                    console.log("⚠️ No handler found for:", key);
                    return;
                }

                await handlers[key]();

                console.log(`✅ Upload + DB updated (${key})`);
                ...
                try {
                    if (payload?.entityType === "user") {
                        await userRepo.update(payload.entityId, {
                            resume_upload_status: "fail",
                        });
                    }

                    if (payload?.entityType === "company") {
                        await companyRepo.update(payload.entityId, {
                            logo_upload_status: "fail",
                        });
                    }

                    console.log("⚠️ Failure status updated in DB");
